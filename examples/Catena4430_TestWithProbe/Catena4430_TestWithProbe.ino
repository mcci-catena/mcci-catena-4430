...
Module: Catena4430_TestWithProbe.ino

Function:
    Simple test program for the Catena 4430, including temperature probe
...
// temperature probe stuff.
static constexpr unsigned PIN_ONE_WIRE = A1;
static constexpr unsigned PIN_PWR_4610_PULLUP = D11;
static constexpr std::uint8_t kPowerProbeMask = 1 << 0;

// this is the low-level bus to the probe
OneWire oneWire(PIN_ONE_WIRE);

// this represents all the probes connected to the specified bus
DallasTemperature sensor_TemperatureProbes(&oneWire);
...
void setup() {
    ...
    // finally, turn on power at the connector for the probe
    gpio.setVout1(true);
}
...
void loop()
    ...
    if (pirPrintTimer.isready())
        {
        // for reaons having to do with rounding and symmetry, pir.read()
        ...
        // measure temperture
        float t = 0.0;
        sensor_TemperatureProbes.begin();
        bool fValid = false;
        if (sensor_TemperatureProbes.getDeviceCount() != 0)
            {
            // start a measurement and wait until done.
            // not polling unless we do some extra work,
            // for test, we don't care.
            sensor_TemperatureProbes.requestTemperatures();

            // get the temperature from the first probe.
            t = sensor_TemperatureProbes.getTempCByIndex(0);

            // check for disconnected or error
