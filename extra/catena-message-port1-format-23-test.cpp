...
Name:   catena-message-port1-format-23-test.cpp

Function:
    Generate test vectors for port 0x01 format 0x23 messages.
...
void encodeMeasurement(Buffer &buf, Measurements &m)
    ...
    if (m.Tprobe.fValid)
        {
        flags |= 1 << 6;
        buf.push_back_be(encodeT(m.Tprobe.v));
        }
...
void logMeasurement(Measurements &m)
    ...
    if (m.Tprobe.fValid)
        {
        std::cout << pad.get() << "Tprobe " << m.Tprobe.v;
        }
