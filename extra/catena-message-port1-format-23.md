# Understanding MCCI Catena data sent on port 1 format 0x23
...
Port 1 format 0x23 uplink messages are sent by Catena4430_Sensor and related sketches. We use the discriminator byte in the same way as many of the sketches in the Catena-Sketches collection.
...
`23 00 00 00 00 01 18 00`

```json
{
  "time": "1980-01-05T23:59:43.000Z"
  "Vbat": 1.5
}
```

`23 00 00 00 00 02 F8 00`
...
`23 00 00 00 00 10 14 00 5f 8f 99 99`
...
`23 00 00 00 00 10 1e 00 63 54 99 99`
...
`23 00 00 00 00 80 7C 3D FF FF 7F FF FC 00 74 00 F4 CD`
...
`23 4a d5 06 db ff 20 00 34 cd 4e 66 2a 1e 00 63 54 99 99 00 c8 22 1a 7c 3d ff ff 7f ff fc 00 74 00 f4 cd`
...
```console
C> cl /EHsc catena-message-port1-format-23-test.cpp
Microsoft (R) C/C++ Optimizing Compiler Version 19.16.27031.1 for x64
Copyright (C) Microsoft Corporation.  All rights reserved.

catena-message-port1-format-23-test.cpp
Microsoft (R) Incremental Linker Version 14.16.27031.1
Copyright (C) Microsoft Corporation.  All rights reserved.

/out:catena-message-port1-format-23-test.exe
catena-message-port1-format-23-test.obj
```

Using GCC or Clang on Linux:

```bash
make catena-message-port1-format-23-test
```

(The default make rules should work.)

For usage, read the source or the check the input vector generation file `catena-message-port1-format-23.vec`.

To run it against the test vectors, try:

```console
C> catena-message-port1-format-23-test < catena-message-port1-format-23.vec
Time 1255474907 .
22 4a d5 06 db 00
length: 6
Vbat 1.5 .
